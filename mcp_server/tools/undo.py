...
the inverse itself is destructive — recorded under its own risk-tier label).
``undo_apply`` is itself governed; the inverse it dispatches is a real audited
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    The inverse runs through its own governed tool, so it is audited and
    labelled with its own risk tier there. Pass dry_run=True to preview the
