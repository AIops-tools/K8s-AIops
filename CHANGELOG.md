...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/k8s-aiops` to
  `@zw008/k8s-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/k8s-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/k8s-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install k8s-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/k8s-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [k8s-aiops, uvx]` — either one suffices — with every variable kept
...
### Fixed
- **`delete_namespace` guards the control-plane namespaces.** `kube-system`, `kube-public` and `kube-node-lease` require an explicit `confirm`; there was no protected list at all.
- `urllib3` is now declared explicitly rather than relied on transitively..
