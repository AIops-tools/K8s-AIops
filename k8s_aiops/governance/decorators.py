"""The ``@governed_tool`` decorator — mandatory wrapper for all Kubernetes MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.k8s-aiops/audit.db``
...
from k8s_aiops.governance.audit import detect_agent, get_engine
from k8s_aiops.governance.budget import BudgetExceeded, get_budget
from k8s_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from k8s_aiops.governance.patterns import PatternMatch, get_pattern_engine
from k8s_aiops.governance.policy import PolicyResult, get_policy_engine
from k8s_aiops.governance.readonly import is_read_only, read_only_denial
from k8s_aiops.governance.sanitize import sanitize

_log = logging.getLogger("k8s-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all Kubernetes MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.k8s-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("K8S_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("K8S_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (K8S_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set K8S_AUDIT_APPROVED_BY to the authorizing human (and "
            f"K8S_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`k8s_aiops.governance.outcome`). The change may be live, so an
    ...
    if not descriptor:
        return
    try:
        from k8s_aiops.governance.undo import get_undo_store
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight Kubernetes calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``k8s_aiops.ops.jobs`` → ``k8s-aiops``
    ``mcp_server.server`` → ``k8s-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("k8s_aiops") or module.startswith("mcp_server"):
        return "k8s-aiops"
    return "unknown"
