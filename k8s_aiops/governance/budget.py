...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``K8S_RUNAWAY_MAX`` times within ``K8S_RUNAWAY_WINDOW_SEC`` trips a
...
from k8s_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing Kubernetes-specific; the decorator wraps it into a
