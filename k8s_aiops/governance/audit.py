"""Unified audit logging engine — all Kubernetes skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.k8s-aiops/audit.db``.
...
from k8s_aiops.governance.paths import ops_path

_log = logging.getLogger("k8s-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring K8S_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the K8S_AIOPS_HOME refactor
