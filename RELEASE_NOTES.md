...
With this set the **16 write tools are never registered** — an MCP
client lists **39 tools instead of 55**. The writes are not hidden
...
Two new read-only analyses — `pod_health_rca` and `workload_readiness_rca` — plus a
...
- **`skills/k8s-aiops/references/agent-guardrails.md`** — for driving this tool with a
