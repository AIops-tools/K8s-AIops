# Release notes — k8s-aiops 0.7.0

Previous release: 0.6.0.

## In this tool

- **`delete_namespace` guards the control-plane namespaces.** `kube-system`, `kube-public` and `kube-node-lease` require an explicit `confirm`; there was no protected list at all. Separately, when the kubeconfig authenticates as a ServiceAccount, deleting that SA's own namespace revokes this tool's credential — that case is refused outright and `confirm` does not override it. Identity is read from the kubeconfig on disk, so it costs no cluster call and works on the preview path; cert-based and exec/`auth-provider` contexts proceed as before.
