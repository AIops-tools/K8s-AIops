...
throwaway home (``K8S_AIOPS_HOME``) and assert that the rows compliance
evidence is built from actually land on disk — including that a high-risk write
runs and is audited with no approver required, since the skill authorizes
nothing.
"""
...
from k8s_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("K8S_AUDIT_APPROVED_BY", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_an_approver_is_still_recorded_when_supplied(gov_home, monkeypatch):
    ...
    monkeypatch.setenv("K8S_AUDIT_APPROVED_BY", "sre-alice")
...
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: scale_deployment must
    capture the previous replica count and persist the scale-back undo on disk."""
    ...
    result = gov.scale_deployment(name="web", replicas=5, namespace="prod")
    assert result["previous_replicas"] == 3
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "scale_deployment"
    assert json.loads(undo[0]["undo_params"])["replicas"] == 3  # the captured prior count

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["scale_deployment"]
