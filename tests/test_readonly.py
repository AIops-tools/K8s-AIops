...
from k8s_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_governed_write_is_refused_in_read_only(read_only):
    """A write is refused before it can reach the apiserver — from any caller."""
    calls = []

    @governed_tool(risk_level="high")
    def delete_everything(namespace: str = "prod") -> dict:
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def list_pods(namespace: str = "prod") -> list:
        return [{"name": "web-0"}]

    assert list_pods() == [{"name": "web-0"}]


@pytest.mark.unit
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. undo_apply, scale) must be refused."""

    @governed_tool(risk_level="medium")
    def scale_deployment(namespace: str = "prod") -> dict:
        return {"scaled": True}
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "delete_pod" in dropped and "scale_deployment" in dropped
        assert "drain_node" in dropped and "delete_namespace" in dropped
        assert "pod_list" not in dropped and "pod_logs" not in dropped
        ...
        assert "pod_list" in remaining, "reads must still be exposed"
        assert "event_list" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
